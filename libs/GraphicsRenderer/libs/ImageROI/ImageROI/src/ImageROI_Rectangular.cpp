#include "./ImageROI_Rectangular.hpp"

#include <QtMath>
#include <QDebug>


ImageROI_Rectangular::ImageROI_Rectangular(ImageROI_Base *parent):
    m_bResize(false),
...
QRectF ImageROI_Rectangular::boundingRect() const
{
...
ImageROI_Rectangular::~ImageROI_Rectangular()
{
    delete []pPointFofSmallRotateRect;
    pPointFofSmallRotateRect = nullptr;
}

void ImageROI_Rectangular::setRectSize(QRectF mrect, bool bResetRotateCenter)
{
    m_oldRect = mrect;
...
void ImageROI_Rectangular::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    QPen mPen = QPen( QString::fromStdString( Get_Color() ) );
    ...
    //绘制旋转圆形
    mPen.setColor(  QString::fromStdString( Get_Color() ) );
    painter->setPen(mPen);
...
void ImageROI_Rectangular::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
...
void ImageROI_Rectangular::mouseMoveEvent(QGraphicsSceneMouseEvent *event)
{
...
void ImageROI_Rectangular::mouseReleaseEvent(QGraphicsSceneMouseEvent *event)
{
...
void ImageROI_Rectangular::SetRotate(qreal RotateAngle, QPointF ptCenter)
{
    m_bRotate = true;
...
QPointF ImageROI_Rectangular::getRotatePoint(QPointF ptCenter, QPointF ptIn, qreal angle)
{
...
QList<QPointF> ImageROI_Rectangular::getRotatePoints(QPointF ptCenter, QList<QPointF> ptIns, qreal angle)
{
    QList<QPointF> lstPt;
...
QPolygonF ImageROI_Rectangular::getRotatePolygonFromRect(QPointF ptCenter, QRectF rectIn, qreal angle)
{
    QVector<QPointF> vpt;
...
QRectF ImageROI_Rectangular::getCrtPosRectToSceen()
{
    QRectF retRect = QRectF(m_oldRect.x()+pos().x(),m_oldRect.y()+pos().y(),m_oldRect.width(),m_oldRect.height());
    return retRect;
}
QRectF ImageROI_Rectangular::getSmallRotateRect(QPointF ptA,QPointF ptB)
{
    QPointF pt = getSmallRotateRectCenter(ptA,ptB);
    return QRectF(pt.x()-10,pt.y()-10,20,20);
}
QPointF ImageROI_Rectangular::getSmallRotateRectCenter(QPointF ptA,QPointF ptB)
{
